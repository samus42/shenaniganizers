...
const ClanRoster = ({excludeList = [], onSelect = () => {}, disabled = false}) => {
    ...
    useEffect(() => {
        const getRoster = async () => {
            try {
                ...
                setLoadError(
                    `Destiny API is currently having issues, so we cannot display the clan roster. It's not my fault!`
                )
            }
        }
        getRoster()
    ...
    if (loadError) {
        return (
            <div style={{paddingTop: '20px', paddingLeft: '10px', maxWidth: '200px', color: 'red'}}>
