                ...
                >
                    ...
                    <Stack direction="row" alignItems="center" justifyContent="space-between">
                        <div style={{width: '250px'}}>
                            <TextField
                                fullWidth
                                variant="filled"
                                label="Create a name for this run!"
                                value={instanceName}
                                onChange={(evt) =>
                                    onDetailsChange({instanceName: evt.target.value, date})
                                }
                            />
                        </div>
                        <div>
                        ...
                        <div style={{display: 'flex', flexDirection: 'column'}}>
                            <Button
                                ...
                                variant="contained"
                                disabled={instanceName.length < 1 || !date}
                    ...
                    <div style={{paddingLeft: '0px', paddingTop: '10px'}}>
                        <div style={{paddingBottom: '10px'}}>
                            {`Drag and drop players onto slots. Once a player is in a slot, you can
                            use that player as the starting point to drag to another stage of the
                            raid (so you don't always have to drag from the left bar). If you drag a
                            player over another in the same stage, they will swap roles. Or you can
                            just hit the randomize roles button and let luck decide!`}
                        </div>
                        <Button
                            variant="contained"
                            disabled={roster.length < 6}
                            onClick={determineRoles}
                        >
                            Randomize Roles
                        </Button>
                    </div>
                    ...
                    <RaidAssignments
                        raid={raid}
                        onChange={onRaidPositionChange}
                        onStrategyChange={onRaidStageStrategyChange}
                    />
                    {raid.id && (
                        <div style={{paddingTop: '10px', paddingBottom: '10px'}}>
                            <Button onClick={onArchive} variant="contained">
                                Archive Raid To Remove From Active List
                            </Button>
                        </div>
                    )}
                </div>
