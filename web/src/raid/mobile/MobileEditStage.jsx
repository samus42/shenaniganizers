    ...
    return (
        ...
        >
            ...
            <div style={{display: 'flex', justifyContent: 'space-between', minWidth: '150px'}}>
                <select
                    onChange={(evt) => onChange(role, evt.target.value)}
                    style={{fontSize: '18px'}}
                    value={role.player ? role.player.destinyId : -1}
                >
                    {[
                        ...
                        raid.roster.map((player) => (
                            <option key={player.destinyId} value={player.destinyId}>
                                {player.name}
                            </option>
                        ))
                    )}
    ...
    return (
        <>
            <Dialog fullScreen open={open} onClose={() => setOpen(false)}>
                ...
                <DialogContent>
                    <div>
                        {workingStage.roles.map((role) => (
                            <Role
                                key={`role-${role.name}`}
                                raid={raid}
                                role={role}
                                onChange={onRolePlayerChange}
                            />
                        ))}
                    </div>
                </DialogContent>
